...
def print_version_info():
	version = "{} v{} by {} -> (Github: {})".format(Constants.PROJECT_NAME.value,
													Constants.VERSION.value,
													Constants.AUTHOR_FULL_NAME.value,
													Constants.AUTHOR_GITHUB.value)
	line = "-" * (len(version))
...
def splash_screen():
	...
	"""
	print(Fore.YELLOW + Style.BRIGHT + "\n" + Constants.LOGO.value + Style.RESET_ALL)
	print_version_info()
...
def _copy_dir(source_dir, backup_path):
	...
	"""
	invalid = set(Constants.INVALID_DIRS.value)
...
def _home_prefix(path):
	return os.path.join(Constants.HOME_DIRECTORY.value, path)
...
def get_configs_path_mapping():
	...
	"""
	return {
		"{}/Packages/User/".format(Constants.SUBLIME2_DIRECTORY.value): "sublime_2",
		"{}/Packages/User/".format(Constants.SUBLIME3_DIRECTORY.value): "sublime_3",
		Constants.INTELLIJ_DIRECTORY.value: "intellijidea_2018.2",
		Constants.PYCHARM_DIRECTORY.value: "pycharm_2018.2",
		Constants.CLION_DIRECTORY.value: "clion_2018.2",
		Constants.PHPSTORM_DIRECTORY.value: "phpstorm_2018.2",
		Constants.ATOM_DIRECTORY.value: "atom",
	}
...
def backup_dotfiles(backup_path):
	...
	dotfiles = [file for file in dotfiles_for_backup if os.path.isfile(
		os.path.join(Constants.HOME_DIRECTORY.value, file))]
	dotfolders = [folder for folder in dotfolders_for_backup if os.path.exists(
		os.path.join(Constants.HOME_DIRECTORY.value, folder))]
	...
	for dotfolder in dotfolders:
		dotfolders_mp_in.append(
			(os.path.join(Constants.HOME_DIRECTORY.value, dotfolder), backup_path))

	dotfiles_mp_in = []
	for dotfile in dotfiles:
		dotfiles_mp_in.append((os.path.join(Constants.HOME_DIRECTORY.value, dotfile), os.path.join(backup_path, dotfile)))
...
def backup_packages(backup_path):
	...
	sublime_2_path = _home_prefix("{}/Packages/".format(Constants.SUBLIME2_DIRECTORY.value))
	...
	sublime_3_path = _home_prefix("{}/Installed Packages/".format(Constants.SUBLIME3_DIRECTORY.value))
	...
	command = "ls {}".format(Constants.APPLICATIONS_DIRECTORY.value)
...
def backup_fonts(path):
	...
	fonts_path = Constants.FONTS_DIRECTORY.value
...
def reinstall_packages_from_lists(packages_path):
	...
	for file in os.listdir(packages_path):
		...
		if manager in Constants.PACKAGE_MANAGERS.value:
...
def get_config_path():
	return _home_prefix(Constants.CONFIG_PATH.value)
