...
def make_dir_warn_overwrite(path):
	...
	if os.path.exists(path) and path.split("/")[-1] in ["dotfiles", "packages", "fonts", "configs"]:
		...
		if prompt_yes_no("Erase directory and make new back up?", Fore.RED):
			...
			print(
					Fore.RED + "Exiting to prevent accidental deletion of user data." + Style.RESET_ALL)
			sys.exit()
...
def backup_dotfiles(backup_path):
	...
	if os.path.isdir("/Users/alichtman/Library/Application Support/Sublime\ Text\ 2"):
		dotfolders_mp_in.append(
			(os.path.join(home_path, "Library/Application\ Support/Sublime\ Text\ 2/Packages/User"), backup_path))

	if os.path.isdir("/Users/alichtman/Library/Application Support/Sublime\ Text\ 3"):
		dotfolders_mp_in.append(
			(os.path.join(home_path, "Library/Application\ Support/Sublime\ Text\ 3/Packages/User"), backup_path))
...
def backup_configs(backup_path):
	...
	for config, target in configs_dir_mapping.items():
		if os.path.isdir(home_prefix(config)):
			configs_backup_path = os.path.join(backup_path, target)
			_mkdir_or_pass(configs_backup_path)
			_copy_dir_content(home_prefix(config), configs_backup_path)
	...
	for plist in plist_files:
		if os.path.exists(home_prefix(plist)):
			_copy_dir_content(home_prefix(plist), plist_backup_path)
...
def reinstall_config_files(configs_path):
	...
	for target, backup in configs_dir_mapping.items():
		if os.path.isdir(backup_prefix(backup)):
			_copy_dir_content(backup_prefix(backup), home_prefix(target))

	for target, backup in plist_files.items():
		if os.path.exists(backup_prefix(backup)):
			_copy_file(backup_prefix(backup), home_prefix(target))
...
def cli(complete, dotfiles, configs, packages, fonts, old_path, new_path, reinstall, reinstall_configs, delete_config,
	...
	if not os.path.exists(_config_path):
		print(Fore.BLUE + Style.BRIGHT +
			  "Creating config file at {}".format(_config_path))
